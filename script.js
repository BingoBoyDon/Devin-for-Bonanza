...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Show the new game confirmation modal when the page loads
    const newGameModal = document.getElementById('new-game-modal');
    if (newGameModal) {
        newGameModal.style.display = 'block';
        
        // Handle Yes button click
        document.getElementById('new-game-yes-button').addEventListener('click', function() {
            // Close the modal
            newGameModal.style.display = 'none';
            
            // Get the MAC address and reset all numbers
            if (typeof getMacAddress === 'function' && typeof sendResetAllMessages === 'function') {
                getMacAddress(siteId).then(mac => {
                    if (mac) {
                        macAddress = mac;
                        
                        // Send reset messages for all numbers
                        sendResetAllMessages(macAddress);
                        
                        // Send text message with "new game number"
                        if (typeof sendTextMessage === 'function') {
                            const gameNumber = document.getElementById('game-number').textContent;
                            sendTextMessage(`New Game Number ${gameNumber}`, macAddress);
                        } else {
                            console.error("sendTextMessage function is not defined");
                        }
                    }
                });
            } else {
        ...
        document.getElementById('new-game-no-button').addEventListener('click', function() {
            // Close the modal
            newGameModal.style.display = 'none';
            
            // Disable all bingo number buttons
            ...
            // Hide the Reset Selected button
            const resetBingoButton = document.getElementById('reset-bingo-button');
            if (resetBingoButton) {
                resetBingoButton.style.display = 'none';
            }
            
            // Show a message to the user
            showMessage("Game buttons have been disabled. This is not a new game.", "info", 5000);
        });
    }
});
                        ...
                        getMacAddress(siteId).then(mac => {
                            if (mac) {
                                ...
                                if (!this.classList.contains('marked') && sequence.length < 30) {
                                    ...
                                    // Enviar mensaje de texto con el cellId
                                    if (typeof sendTextMessage === "function") {
                                        sendTextMessage(cellId, macAddress);
                                    }
                                }
                            }
                        });
