...
class TestServiceServicer(test_pb2_grpc.TestServiceServicer):
    ...
    def StreamBalanceAdjustment(self, request, context):
        ...
        try:
            ...
            while context.is_active():
                # Buscar ajustes pendientes para este cliente
                cursor.execute("""
                ...
                if result:
                    adjustment_id, costumers_id, amount, phone_number = result
                    
                    # Enviar solicitud de ajuste al cliente
                    yield test_pb2.BalanceAdjustmentRequest(
                        client_id=client_id,
                        costumers_id=costumers_id,
                        adjustment_id=adjustment_id,
                        adjustment_requested_amount=amount,
                        phone_number=phone_number
                    )
                    
                    logger.info(f"Sent balance adjustment request to client {client_id}: AdjustmentID {adjustment_id}, Amount {amount}")
    ...
    def ConfirmBalanceAdjustment(self, request, context):
        ...
        try:
            ...
            result = subprocess.run(
                ["python3", "/home/lynxx/azkiosk_client/balance_db_updater.py"],
